...
from utils.cotrack_utils import filter_trajs, viz_preds, compute_loss, viz_loss, viz_vel_render, compute_vel_loss
...
def get_mask(depth_splat_ids,n_gaussians,projections = None,w=800,h=800):
    unique_depth_splat_ids = torch.unique(depth_splat_ids) 
    unique_depth_splat_ids = unique_depth_splat_ids[unique_depth_splat_ids != -1]
    mask = torch.zeros(n_gaussians,device='cuda',dtype=torch.bool)
    mask[unique_depth_splat_ids.long()] = 1

    if projections is not None:
        # check if projections are within the image
        in_img_mask = torch.tensor((projections[:,0] >= 0) & (projections[:,0] < w) & (projections[:,1] >= 0) & (projections[:,1] < h),device='cuda',dtype=torch.bool)
        mask = mask & in_img_mask
    return mask
...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background,log_deform=False,args=None,gt=None,force_colors=None):
    ...
    for idx in (tqdm(range(len(views)), desc="Rendering progress")):
        view = views[idx]
        previous_view = views[idx-1] if idx > 0 else None
        ...
        render_pkg = render(view, gaussians, pipeline, background,log_deform_path=log_deform_path,no_shadow=args.no_shadow,override_color=force_colors,prev_projections=prev_projections)
        ...
        if args.save_depth:
            normalized_depth = (depth - min_depth_viz) / (max_depth_viz - min_depth_viz)
            depth_list.append(normalized_depth)
        ...
        if args.show_cotrack:
            rendering =  np.ascontiguousarray(rendering)
            rendering_clean = rendering.copy()
            if prev_projections is not None:
                if view_id != view.view_id or idx%5 ==0:
                    prev_projections = None
                    prev_image = None
                    all_trajs = None
                    all_projections = None
                    ...
                    preds = torch.tensor(filter_trajs([previous_view,view]),device="cuda") 
                    all_projections = torch.stack([torch.tensor(prev_projections,device="cuda"),render_pkg["projections"]],dim=0)
                    #loss, splat_ids = compute_loss(preds,all_projections,render_pkg["depth_splat_ids"])
                    #loss = signal_to_colors(loss,flat_array=True,mode='meanstd',n_stds=1)
                    #for i in range(splat_ids.shape[0]):
                        #if splat_ids[i] != -1:
                            #loss_color = np.array([loss[i].cpu().numpy(),0,0])
                            ##loss_color = [0.5,0,0]
                            ##cv2.circle(rendering,(int(render_pkg["projections"][splat_ids[i],0]),int(render_pkg["projections"][splat_ids[i],1])),2,loss_color,-1)
                            #projection = render_pkg["projections"][splat_ids[i]]
                            #if projection[0] >= 0 and projection[0] < view.image_width and projection[1] >= 0 and projection[1] < view.image_height:
                                ##rendering[int(projection[1]),int(projection[0]),:] = loss_color
                                #cv2.circle(rendering,(int(render_pkg["projections"][splat_ids[i],0]),int(render_pkg["projections"][splat_ids[i],1])),2,loss_color,-1)  
            if prev_image is not None and preds is not None:
                image = viz_vel_render(rendering_clean,render_pkg["rendered_velocity"],cotrack_preds=preds)
                viz_list.append(image)
                #err = compute_vel_loss(render_pkg["rendered_velocity"],preds)
                #print(err)
                #breakpoint()

                #viz_loss(all_projections[:,splat_ids.long()],preds,loss,image=prev_image)


            prev_projections = render_pkg["projections"].cpu().numpy()
            prev_image = rendering_clean
            ...
            if all_trajs is None:
                ...
                all_projections = render_pkg["projections"].to("cpu").numpy()[gt_idxs] 
            else:
                ...
                all_projections = np.concatenate((all_projections,render_pkg["projections"].to("cpu").numpy()[gt_idxs]),axis=0)
            ...
            current_mask = get_mask(render_pkg["depth_splat_ids"],n_gaussians=n_gaussians,projections=current_projections,w=view.image_width,h=view.image_height)
    ...
    if args.save_depth:
        depth_imgs = [to8(img).transpose(1,2,0) for img in depth_list]
        imageio.mimwrite(os.path.join(model_path, name, "ours_{}".format(iteration), 'video_depth.mp4'), depth_imgs, fps=30, quality=8)

    print("writing viz images.")
    if len(viz_list) != 0:
        imageio.mimwrite(os.path.join(model_path, name, "ours_{}".format(iteration), 'video_viz.mp4'), viz_list, fps=2, quality=8)
...
def signal_to_colors(signal,mode='minmax',threshold=None,flat_array=False,n_stds=1):
    ...
    if threshold is not None:
        ...
        if mode == 'minmax':
            ...
            signal = (signal - torch.mean(signal)) / (torch.std(signal)*n_stds) + 0.5
            # clip
...
if __name__ == "__main__":
    ...
    parser.add_argument("--show_cotrack",action="store_true")
