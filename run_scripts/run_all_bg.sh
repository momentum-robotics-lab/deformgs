...
do
    ...
    do 
        python3 train.py -s "data/final_scenes_bg/${SCENE}" --port $port --expname "final_scenes_bg_iso_0.01/${SCENE}" --configs arguments/mdnerf-dataset/cube.py --lambda_w 100000 \
        ...
        --wandb_name "iso_0.01" --k_nearest 5 --lambda_isometric $isometry --time_skip 1
        # add one to port
        port=$((port+1))
    done
done
